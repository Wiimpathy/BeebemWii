...
//	for (loop=0; loop<1024; loop++) {
  ...
  if ((NMIStatus) && (!OldNMIStatus)) DoNMI();
	...
	if (current_time != previous_time) {
		...
		printf("frame = %d, second = %d, fps = %d, slept = %d, total cycles = %08x, cycles last second = %08x, sound buffered = %ld\n", mainWin->currentframe(), current_time, mainWin->currentframe() - previous_framecount,  time_slept, TotalCycles, TotalCycles-previous_cycles, HowManyBytesLeftInSDLSoundBuffer());
		previous_cycles = TotalCycles;
		previous_framecount = mainWin->currentframe();
