...
const char* GetDiscImagePath(int drive)
{
	...
	return (const char*) FileNames[drive];
}
...
//--#else
 ...
 char errmes[5*1024];
	strcpy(errmes, NotImpCom);
	strcat(errmes, " has not been implemented in disc8271 - sorry!");

 EG_MessageBox(frame_buffer_p, EG_MESSAGEBOX_STOP, "Not implemented:", errmes, "Exit BeebEm", NULL, NULL, NULL, 0);
...
//--#ifdef WIN32
    char errstr[200];
    sprintf(errstr, DISC8271_LOADDISC_M, FileName);
//    MessageBox(GETHWND,errstr,"BBC Emulator",MB_OK|MB_ICONERROR);
//--#else
   cerr << "Could not open disc file: " << FileName << "\n";
//--#endif

	EG_MessageBox(frame_buffer_p, EG_MESSAGEBOX_STOP, DISC8271_LOADDISC_T, errstr, "Continue", NULL, NULL, NULL, 0);
...
//--  mainWin->SetImageName(FileName,DriveNum,0);
  ...
  strcpy(FileNames[DriveNum], FileName);
	...
	if (DriveNum == 0) strcpy(config.drive_0_path, FileName);
	else strcpy(config.drive_1_path, FileName);
...
//--                       "BBC Emulator",MB_OK|MB_ICONWARNING);
	...
	EG_MessageBox(frame_buffer_p, EG_MESSAGEBOX_STOP, DISC8271_INCORRECTFORMAT_T, DISC8271_INCORRECTFORMAT_M, "Continue", NULL, NULL, NULL, 0);
...
//--#else
   ...
   cerr << "Check files before copying them.\n";
	...
	EG_MessageBox(frame_buffer_p, EG_MESSAGEBOX_STOP, DISC8271_INCORRECTFORMAT2_T, DISC8271_INCORRECTFORMAT2_M, "Continue", NULL, NULL, NULL, 0);
...
void Eject8271DiscImage(int DriveNum) {
  strcpy(FileNames[DriveNum], "");
        ...
        if (DriveNum == 0) strcpy(config.drive_0_path, "");
        else strcpy(config.drive_1_path, "");
...
//--#ifdef WIN32
    char errstr[200];
    sprintf(errstr, DISC8271_SAVETRACK_M, FileNames[DriveNum]);
//--    MessageBox(GETHWND,errstr,"BBC Emulator",MB_OK|MB_ICONERROR);
//--#else
    cerr << "Could not open disc file for write " << FileNames[DriveNum] << "\n";
//--#endif

	EG_MessageBox(frame_buffer_p, EG_MESSAGEBOX_STOP, DISC8271_SAVETRACK_T, errstr, "Continue", NULL, NULL, NULL, 0);
...
//--#ifdef WIN32
    char errstr[200];
    sprintf(errstr, DISC8271_SAVETRACK2_M, FileNames[DriveNum]);
//--    MessageBox(GETHWND,errstr,"BBC Emulator",MB_OK|MB_ICONERROR);
//--#else
   cerr << "Failed writing to disc file " << FileNames[DriveNum] << "\n";
//--#endif

	EG_MessageBox(frame_buffer_p, EG_MESSAGEBOX_STOP, DISC8271_SAVETRACK2_T, errstr, "Continue", NULL, NULL, NULL, 0);

  };

#ifndef GEKKO
	// Sync fat32 on GP2X SD card
	sync();
#endif
...
//++>
  /* If WriteEnable is illegal, make RO:
   */
  if (WriteEnable != 0 && WriteEnable != 1) WriteEnable = 0;

  /* Set in settings and handle illegal DriveNum:
   */
  if (DriveNum == 0)
        config.settings.protection_8271_0 = (WriteEnable ? RW_8271 : RO_8271);
  else if (DriveNum == 1)
        config.settings.protection_8271_1 = (WriteEnable ? RW_8271 : RO_8271);
  else return;
//<++
...
//--#endif
	...
	EG_MessageBox(frame_buffer_p, EG_MESSAGEBOX_STOP, DISC8271_INVALIDDISCCATALOGUE_T, DISC8271_INVALIDDISCCATALOGUE_M, "Continue", NULL, NULL, NULL, 0);
    }
...
//--#ifdef WIN32
    char errstr[200];
    sprintf(errstr, DISC8271_CREATEDISCIMAGECANCEL_M, FileName);
//--    if (MessageBox(GETHWND,errstr,"BBC Emulator",MB_YESNO|MB_ICONQUESTION) != IDYES)
 //--     return;
//--#else
//--   cerr << "Could not create disc file " << FileName << "\n";

	if (EG_MessageBox(frame_buffer_p, EG_MESSAGEBOX_QUESTION, DISC8271_CREATEDISCIMAGECANCEL_T, errstr, "Yes", "No", NULL, NULL, 2) == 2) {
		EG_MessageBox(frame_buffer_p, EG_MESSAGEBOX_INFORMATION, DISC8271_OVERWRITEDISCIMAGECANCELLED_T, DISC8271_OVERWRITEDISCIMAGECANCELLED_M, "Continue", NULL, NULL, NULL, 0);
   		return;
	}
//--#endif
  };
...
//--#ifdef WIN32
    char errstr[200];
    sprintf(errstr, DISC8271_COULDNOTCREATEFILE_M, FileName);
//--    MessageBox(GETHWND,errstr,"BBC Emulator",MB_OK|MB_ICONERROR);
//--#else
   cerr << "Could not create disc file '" << FileName << "'.\n";
...
//--#endif
	...
	EG_MessageBox(frame_buffer_p, EG_MESSAGEBOX_INFORMATION, DISC8271_CREATEDFILE_T, DISC8271_CREATEDFILE_M, "Continue", NULL, NULL, NULL, 0);
...
//++
	...
	sprintf(errstr, DISC8271_CREATEFILEFAILEDWRITING_M, FileName);
//<-
//--    MessageBox(GETHWND,errstr,"BBC Emulator",MB_OK|MB_ICONERROR);
//--#else
   cerr << "Failed writing to disc file " << FileName << "\n";
	EG_MessageBox(frame_buffer_p, EG_MESSAGEBOX_STOP, DISC8271_CREATEFILEFAILEDWRITING_T, errstr, "Continue", NULL, NULL, NULL, 0);
...
//--          MessageBox(GETHWND,"The 8271 FDC Cannot load the ADFS disc image specified in the BeebDiscLoad environment variable","BeebEm",MB_ICONERROR|MB_OK);
	...
	cerr << "The 8271 FDC Cannot load the ADFS disc image specified in the BeebDiscLoad environment variable\n";
