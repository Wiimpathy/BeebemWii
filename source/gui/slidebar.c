...
//#define EG_SLIDEBAR_ASTROFFONLY	1
...
#ifndef EG_SLIDEBAR_ASTROFFONLY
...
#else
		if (slidebar_ptr->slider_area.h < 1)
			slidebar_ptr->slider_area.h = 1;
#endif
...
#else
		if (slidebar_ptr->slider_area.w < 1)
			slidebar_ptr->slider_area.w = 1;
#endif
...
//#endif
	...
	/* Draw slide bar with mouse support:
 	 */
#ifndef EG_SLIDEBAR_ASTROFFONLY
...
#else
	/* Draw slidebar without mouse support (troff only):
 	 */
	tmp = loc;
	EG_Draw_Box(surface_ptr, &tmp, &color);
	EG_Draw_Border(surface_ptr,&tmp, &color, EG_Draw_Border_BoxLow);
...
static void InitializePayload(EG_SlideBar *slidebar_ptr, SDL_Color color, int orientation, SDL_Rect area)
{
	...
	slidebar_ptr->slider_label[0] = 10;
...
/* The slide bar can get focus - defaults to this.
 */
void EG_SlideBar_CanGetFocus(EG_Widget *widget_ptr)
{
	/* Set focus control callbacks for widget to
	 * generic focus allowed functions:
	 */
	(void) EG_Widget_SetCallback_GotFocus(widget_ptr
	 , EG_Callback_Generic_GotFocus);
...
/* Once called the slide bar cannot receive focus. If
 * the slidebar already has focus, then focus moves to
 * the next widget. If no other widgets can receive
 * focus, then no widget has focus - focus cleared.
 */
void EG_SlideBar_CannotGetFocus(EG_Widget *widget_ptr)
{
	EG_Window *window_ptr = NULL;

	/* If this widget has focus, then loose focus:
	 */
	if ( (window_ptr = (EG_Window*) EG_Widget_GetWindow(widget_ptr) ) 
	 != NULL) {
		if ( EG_Window_ThisWidgetHasFocus(widget_ptr) == EG_TRUE) {
			if ( EG_Window_MoveFocusForward(window_ptr) == EG_FALSE)
				EG_Window_ClearFocus(window_ptr);
		}
	}

	/* Set focus control callbacks for widget to
	 * generic focus disallowed functions:
	 */
	(void) EG_Widget_SetCallback_GotFocus(widget_ptr
