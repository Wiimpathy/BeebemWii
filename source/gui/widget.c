 ...
 , const char *name_ptr, EG_StringHash type)
{
	...
	/* Smart focus movement is set to on:
	 */
	widget_ptr->just_set_focus = EG_TRUE;

	/* Set smart focus movement mapping to nothing:
	 */
	widget_ptr->focus_left = NULL;
	widget_ptr->focus_right = NULL;
	widget_ptr->focus_up = NULL;
	widget_ptr->focus_down = NULL;

	widget_ptr->i_will_handle_focus = EG_FALSE;
	...
	(void) EG_Widget_SetUserCallback_SmartFocus(widget_ptr, NULL, NULL);
...
void EG_Widget_CallUserSmartFocus(EG_Widget *widget_ptr
 , EG_Focus_Direction direction)
{
	EG_Callback_User_SmartFocus func_ptr;

        CHECK_EG_WIDGET_IS_VALID_VOID(widget_ptr);

        if (widget_ptr->func_user_smartfocus_ptr != NULL){
                func_ptr
                 = (EG_Callback_User_SmartFocus) widget_ptr->func_user_smartfocus_ptr;

                func_ptr(widget_ptr, direction, widget_ptr->user_smartfocus_value_ptr);
        }
}
...
EG_BOOL EG_Widget_SetUserCallback_SmartFocus(EG_Widget *widget_ptr
 , EG_Callback_User_SmartFocus user_callback_ptr, void *user_smartfocus_value_ptr)
{
	CHECK_EG_WIDGET_IS_VALID(widget_ptr, EG_FALSE);

	widget_ptr->func_user_smartfocus_ptr = (void*) user_callback_ptr;
	widget_ptr->user_smartfocus_value_ptr = user_smartfocus_value_ptr;
...
{
        CHECK_EG_WIDGET_IS_VALID_VOID(widget_ptr);
        widget_ptr->func_user_smartfocus_ptr = NULL;
        widget_ptr->user_smartfocus_value_ptr = NULL;
}
...
EG_BOOL EG_Widget_IsSmartFocusAutomated(EG_Widget *widget_ptr)
{
	...
	return widget_ptr->just_set_focus;
}
...
void EG_Widget_SetSmartFocusPeerLeft(EG_Widget *widget_ptr, EG_Widget *peer_ptr)
{
	...
	widget_ptr->focus_left = peer_ptr;
}
...
void EG_Widget_SetSmartFocusPeerRight(EG_Widget *widget_ptr, EG_Widget *peer_ptr)
{
	...
	widget_ptr->focus_right = peer_ptr;
}
...
void EG_Widget_SetSmartFocusPeerDown(EG_Widget *widget_ptr, EG_Widget *peer_ptr)
{
	...
	widget_ptr->focus_down = peer_ptr;
}
...
EG_Widget* EG_Widget_GetSmartFocusPeerLeft(EG_Widget *widget_ptr)
{
	...
	return (EG_Widget*) widget_ptr->focus_left;
}
...
EG_Widget* EG_Widget_GetSmartFocusPeerRight(EG_Widget *widget_ptr)
{
	...
	return (EG_Widget*) widget_ptr->focus_right;
}
...
EG_Widget* EG_Widget_GetSmartFocusPeerUp(EG_Widget *widget_ptr)
{
	...
	return (EG_Widget*) widget_ptr->focus_up;
}
...
EG_Widget* EG_Widget_GetSmartFocusPeerDown(EG_Widget *widget_ptr)
{
	...
	return (EG_Widget*) widget_ptr->focus_down;
}

/* If set then the window will generate a focus/smart focus change
 * event rather than handling focus for this widget (if it has focus).
 * If it's to take focus from another widget, then this is handled
 * in the normal way (it will receive a got focus event).
 *
 * This is added to allow some widgets to have multiple focus points
 * within themselves. Useful for virtual keyboards and such. To loose
 * focus widgets with this state need to handle it themselves.
 */
void	EG_Widget_Focus_SetIWillHandleFocus(EG_Widget *widget_ptr
 , EG_BOOL state)
{
	CHECK_EG_WIDGET_IS_VALID_VOID(widget_ptr);

	widget_ptr->i_will_handle_focus = state;
}

EG_BOOL EG_Widget_Focus_IWillHandleFocusMyself(EG_Widget *widget_ptr)
{
	CHECK_EG_WIDGET_IS_VALID(widget_ptr, EG_FALSE);
	return widget_ptr->i_will_handle_focus;
}
