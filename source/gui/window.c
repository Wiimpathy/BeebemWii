 ...
 */
static EG_Skin_Box     *default_skin_ptr = NULL;
...
static EG_BOOL InitializeWindowStruct(EG_Window *window_ptr,const char *name_ptr)
{
	...
	/* Smart focus is initially unlocked.
	 */
	window_ptr->smart_focus_locked = EG_FALSE;
	...
	EG_Window_SetSkin(window_ptr, NULL);
	EG_Window_UseDefaultSkin(window_ptr, EG_TRUE);
...
/* Handle focus SDL event:
 */
static void RaiseSmartFocusSDLEvent(EG_Widget *widget_ptr, int type)
{
	SDL_Event event;

	if (widget_ptr == NULL) return;

	/* Raise the Smart Focus movement event and pass to the widget:
	 */
	CreateSDLUserEvent(&event, type, 0, 0);
	EG_Widget_CallSDLEvent(widget_ptr, &event);
}

/* Events: Tell the window that it should inform the focused widget that this
 * event happened. This should allow other devices to generate events equal
 * to the mouse and keybaord events.
 *
 * You can use these 'triggers' to map handheld and console buttons to
 * keyboard events.
 */

static void RaiseEvent(EG_Window *window_ptr, int type)
{
	SDL_Event event;
	EG_Widget *widget_ptr;

	/* Get the widget that has focus, if none quit:
	 */
...
/* Select: Can be considered equal to hitting the Enter key and releasing it:
 */
void EG_Window_Trigger_Select_Down(EG_Window *window_ptr)
{
	pDEBUG(dL"Triggering 'select' down for focused widget on '%s'.",dR, EG_Window_GetName(window_ptr) );
	RaiseEvent(window_ptr, EG_USER_SDL_TRIGGER_SELECT_DOWN);
}

void EG_Window_Trigger_Select_Up(EG_Window *window_ptr)
{
	pDEBUG(dL"Triggering 'select' up for focused widget on '%s'.",dR, EG_Window_GetName(window_ptr) );
	RaiseEvent(window_ptr, EG_USER_SDL_TRIGGER_SELECT_UP);
}

/* ----------------------------------------------------------------------------- */

/* Move focus via Smart focus:
 */
void EG_Window_MoveSmartFocusLeft(EG_Window *window_ptr)
{
	EG_Widget *widget_ptr, *peer_ptr;

	qDEBUG("Moving smart focus left.");

	/* Get the widget that has focus, if none quit now:
	 */
	if ( (widget_ptr=EG_Window_Child_GetFocusedWidget(window_ptr)) == NULL)
		return;

	/* If movement is locked (select button down on a widget for example),
	 * then do nothing:
	 */
	if (EG_Window_IsSmartMovementLocked(window_ptr) == EG_TRUE)
		return;

	/* If the focused widget wishes to handle smart focus control internally
	 * (has multiple focus points like a virtual keyboard etc), then
	 * raise an SDL user event to handle this event and pass it to the
	 * child widget.
	 *
	 * Note: Tabbed focus change always passes control to the next widget.
	 */
	if (EG_Widget_Focus_IWillHandleFocusMyself(widget_ptr) == EG_TRUE) {
		printf("*** WARNING: Calling RaiseSmartFocusSDLEvent, smart-focus will not be called.\n");

		RaiseSmartFocusSDLEvent(widget_ptr, EG_USER_SDL_EVENT_SMART_FOCUS_L);
		return;
	}

	/* Get the peer widget we want to switch focus to, if none
	 * quit now:
	 */
	if ( (peer_ptr=EG_Widget_GetSmartFocusPeerLeft(widget_ptr)) == NULL)
		return;

	/* If the widget with focus wants to automate the focus change,
	 * then handle the focus change, otherwise call the widget with
	 * focuses user callback:
	 */
	if ( EG_Widget_IsSmartFocusAutomated(widget_ptr) == EG_TRUE) {
		/* Set focus to new widget. If focus is not excepted then
		 * set focus back to the original widget:
		 */
		if (EG_Window_SetFocusToThisWidget(peer_ptr) == EG_FALSE)
			EG_Window_SetFocusToThisWidget(widget_ptr);
		// [TODO] else { Set peer_ptr's RIGHT peer to widget_ptr }
		else //if (EG_Widget_GetType(peer_ptr) != EG_Widget_Type_TabPage)
			EG_Widget_SetSmartFocusPeerRight(peer_ptr, widget_ptr);

	} else {
		/* Call the callback (the function we call handles NULL):
	 	 */
		EG_Widget_CallUserSmartFocus(widget_ptr, EG_SmartFocus_Left);
	}
}
...
void EG_Window_MoveSmartFocusRight(EG_Window *window_ptr)
{
	...
	qDEBUG("Moving smart focus right.");
	 ...
	 */
	if (EG_Widget_Focus_IWillHandleFocusMyself(widget_ptr) == EG_TRUE) {
		RaiseSmartFocusSDLEvent(widget_ptr, EG_USER_SDL_EVENT_SMART_FOCUS_R);
		return;
	}
	 ...
	 * quit now:
	 */
	if ( (peer_ptr=EG_Widget_GetSmartFocusPeerRight(widget_ptr)) == NULL)
		return;
	...
	if ( EG_Widget_IsSmartFocusAutomated(widget_ptr) == EG_TRUE) {
		...
		if (EG_Window_SetFocusToThisWidget(peer_ptr) == EG_FALSE)
			EG_Window_SetFocusToThisWidget(widget_ptr);
		// [TODO] else { Set peer_ptr's LEFT peer to widget_ptr }
		else //if (EG_Widget_GetType(peer_ptr) != EG_Widget_Type_TabPage)
			EG_Widget_SetSmartFocusPeerLeft(peer_ptr, widget_ptr);
	} else {
		/* Call the callback (the function we call handles NULL):
	 	 */
		EG_Widget_CallUserSmartFocus(widget_ptr, EG_SmartFocus_Right);
	}
}
...
void EG_Window_MoveSmartFocusUp(EG_Window *window_ptr)
{
	 ...
	 */
	if (EG_Widget_Focus_IWillHandleFocusMyself(widget_ptr) == EG_TRUE) {
		RaiseSmartFocusSDLEvent(widget_ptr, EG_USER_SDL_EVENT_SMART_FOCUS_U);
		return;
	}
	 ...
	 * quit now:
	 */
	if ( (peer_ptr=EG_Widget_GetSmartFocusPeerUp(widget_ptr)) == NULL)
		return;
	...
	if ( EG_Widget_IsSmartFocusAutomated(widget_ptr) == EG_TRUE) {
		...
		if (EG_Window_SetFocusToThisWidget(peer_ptr) == EG_FALSE)
			EG_Window_SetFocusToThisWidget(widget_ptr);
		// [TODO] else { Set peer_ptr's DOWN peer to widget_ptr }
		else
			//if (EG_Widget_GetType(peer_ptr) != EG_Widget_Type_TabPage)
				EG_Widget_SetSmartFocusPeerDown(peer_ptr, widget_ptr);
	} else {
		/* Call the callback (the function we call handles NULL):
	 	 */
		EG_Widget_CallUserSmartFocus(widget_ptr, EG_SmartFocus_Up);
	}
}
...
void EG_Window_MoveSmartFocusDown(EG_Window *window_ptr)
{
	...
	qDEBUG("Moving smart focus down.");
	 ...
	 */
	if (EG_Widget_Focus_IWillHandleFocusMyself(widget_ptr) == EG_TRUE) {
		RaiseSmartFocusSDLEvent(widget_ptr, EG_USER_SDL_EVENT_SMART_FOCUS_D);
		return;
	}
	 ...
	 * quit now:
	 */
	if ( (peer_ptr=EG_Widget_GetSmartFocusPeerDown(widget_ptr)) == NULL)
		return;
	...
	if ( EG_Widget_IsSmartFocusAutomated(widget_ptr) == EG_TRUE) {
		...
		if (EG_Window_SetFocusToThisWidget(peer_ptr) == EG_FALSE)
			EG_Window_SetFocusToThisWidget(widget_ptr);
		// [TODO] else { Set peer_ptr's UP peer to widget_ptr }
		else //if (EG_Widget_GetType(peer_ptr) != EG_Widget_Type_TabPage)
			EG_Widget_SetSmartFocusPeerUp(peer_ptr, widget_ptr);
	} else {
		/* Call the callback (the function we call handles NULL):
	 	 */
		EG_Widget_CallUserSmartFocus(widget_ptr, EG_SmartFocus_Down);
	}
}
 ...
 *
 * There is no offset now. All widgets are absolute to the sdl surface
 */
Uint16 EG_Window_GetXDisplacement(EG_Window *window_ptr)
{
	CHECK_EG_WINDOW_IS_VALID(window_ptr, 0);
//	return(window_ptr->dimension.x +2);	
	return(window_ptr->dimension.x);
}
...
/* Lock movement:
 */
void EG_Window_LockSmartMovement(EG_Window *window_ptr)
{
	CHECK_EG_WINDOW_IS_VALID_VOID(window_ptr);

	window_ptr->smart_focus_locked = EG_TRUE;
}

/* Unlock movement:
 */
void EG_Window_UnlockSmartMovement(EG_Window *window_ptr)
{
	CHECK_EG_WINDOW_IS_VALID_VOID(window_ptr);
...
EG_BOOL EG_Window_IsSmartMovementLocked(EG_Window *window_ptr)
{
	...
	return window_ptr->smart_focus_locked;
}
...
//	}else{
		...
		if (window_ptr->skin_box_ptr != NULL) {
			EG_Skin_Box_Paint(window_ptr->skin_box_ptr, surface_ptr, size.x, size.y, size.w, size.h);
		} else {
			if (window_ptr->skin_window && default_skin_ptr != NULL) {
				EG_Skin_Box_Paint(default_skin_ptr, surface_ptr, size.x, size.y, size.w, size.h);
			} else {
				EG_Draw_Box(surface_ptr, &size, &color);
...
//
//
//	// Handle smart focus change keys:
//
//	if ( event_ptr->type==SDL_KEYDOWN
//	 && ( (EG_IsUpsideDown()==EG_FALSE && event_ptr->key.keysym.sym == SDLK_LEFT)
//	      || (EG_IsUpsideDown() == EG_TRUE && event_ptr->key.keysym.sym==SDLK_RIGHT) ) ) {
//		EG_Window_MoveSmartFocusLeft(window_ptr);
//		return EG_TRUE;
//	}
//	if ( event_ptr->type==SDL_KEYDOWN
//	 && ( (EG_IsUpsideDown()==EG_FALSE && event_ptr->key.keysym.sym == SDLK_RIGHT)
//	      || (EG_IsUpsideDown() == EG_TRUE && event_ptr->key.keysym.sym==SDLK_LEFT) ) ) {
//		EG_Window_MoveSmartFocusRight(window_ptr);
//		return EG_TRUE;
//	}
//	if ( event_ptr->type==SDL_KEYDOWN
//	 && ( (EG_IsUpsideDown()==EG_FALSE && event_ptr->key.keysym.sym == SDLK_UP)
//	      || (EG_IsUpsideDown() == EG_TRUE && event_ptr->key.keysym.sym==SDLK_DOWN) ) ) {
//		EG_Window_MoveSmartFocusUp(window_ptr);
//		return EG_TRUE;
//	}
//	if ( event_ptr->type==SDL_KEYDOWN
//	 && ( (EG_IsUpsideDown()==EG_FALSE && event_ptr->key.keysym.sym == SDLK_DOWN)
//	      || (EG_IsUpsideDown() == EG_TRUE && event_ptr->key.keysym.sym==SDLK_UP) ) ) {
//		EG_Window_MoveSmartFocusDown(window_ptr);
//		return EG_TRUE;
//	}
//
//	
//	// [TODO] Handle smart focus change via joystick:
//	
...
/* This just wraps the private widget function as a public function
 * that returns the correct datatype.
 */
EG_Window* EG_Widget_GetWindowPtr(EG_Widget *widget_ptr)
{
	return (EG_Window*) EG_Widget_GetWindow(widget_ptr);
}

/* Create an SDL user event:
 */

int GetSDLUserEventValue1(SDL_Event *event_p)
{
	if (event_p != NULL)
		return (int) ((intptr_t) event_p->user.data1);
	else
		return 0;
}
...
{
	if (event_p != NULL)
		return (int) ((intptr_t) event_p->user.data2);
	else
		return 0;
}

/* Only creates the SDL event structure. If you want to pass this
 * event to a widget use: EG_Widget_CallSDLEvent.
 *
 * If you want window.c to handle the event next time round via
 * the SDL event queue then once the event is created call:
 * SDL_PushEvent(event_p);
 *
 * Most of the time you'll probably just want to call
 * EG_Widget_CallSDLEvent though.
 */
void CreateSDLUserEvent(SDL_Event *event_p, int type, int user_value1
 , int user_value2)
{
	memset(event_p, 0, sizeof(SDL_Event));

	event_p->type = SDL_USEREVENT;
	event_p->user.code = type;

	event_p->user.data1 = (void*) ((intptr_t) user_value1);
	event_p->user.data2 = (void*) ((intptr_t) user_value2);
}
 ...
 , int x_displacement, int y_displacement)
{
	...
	/* Quit with error if window is hidden:
	 */
	if (EG_Window_IsHidden(window_ptr)==EG_TRUE) {
		EG_Log(EG_LOG_DEBUG, dL"EG_Window is hidden, cannot process SDL event.", dR);
		return EG_FALSE;
	}

	if (event_ptr == NULL){
		// [TODO] Use CreateSDLUserEvent for this.
...
EG_BOOL EG_Window_Show(EG_Window *window_ptr)
{
	...
	/* Clear smart focus locking (it's better then it jaming):
	 */
...
EG_BOOL EG_Window_SetSkin(EG_Window *window_ptr, EG_Skin_Box *box_ptr)
{
	...
	window_ptr->skin_box_ptr = box_ptr;
...
{
	default_skin_ptr = box_ptr;
}

EG_Skin_Box* EG_Window_GetDefaultSkin(void)
{
	return default_skin_ptr;
}


void EG_Window_ClearDefaultSkin()
{
	EG_Window_SetDefaultSkin(NULL);
}
