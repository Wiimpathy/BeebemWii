...
static EG_BOOL Callback_Paint(EG_Widget *widget_ptr, SDL_Rect area)
{
	...
	if (tickbox_ptr->skin_ptr != NULL) {
		bold=EG_FALSE;
	
		if (! EG_TickBox_IsTicked(widget_ptr)) {	
			if (tickbox_ptr->depressed == EG_TRUE) {
				EG_Skin_Button_Paint(tickbox_ptr->skin_ptr, surface_ptr, 2, loc.x, loc.y, loc.w, loc.h);
				c_p = NULL;
			} else if (EG_Window_ThisWidgetHasFocus(widget_ptr) == EG_TRUE) {
				EG_Skin_Button_Paint(tickbox_ptr->skin_ptr, surface_ptr, 1, loc.x, loc.y, loc.w, loc.h);
				c_p = &color;
			} else {
				EG_Skin_Button_Paint(tickbox_ptr->skin_ptr, surface_ptr, 0, loc.x, loc.y, loc.w, loc.h);
				c_p = &color;
			}
		} else {

			if (tickbox_ptr->depressed == EG_TRUE) {
				EG_Skin_Button_Paint(tickbox_ptr->skin_ptr, surface_ptr, 5, loc.x, loc.y, loc.w, loc.h);
				c_p = NULL;
			} else if (EG_Window_ThisWidgetHasFocus(widget_ptr) == EG_TRUE) {
				EG_Skin_Button_Paint(tickbox_ptr->skin_ptr, surface_ptr, 4, loc.x, loc.y, loc.w, loc.h);
				c_p = &color;
			} else {
				EG_Skin_Button_Paint(tickbox_ptr->skin_ptr, surface_ptr, 3, loc.x, loc.y, loc.w, loc.h);
				c_p = &color;
			}
		}

		loc.x+=4;
		EG_Draw_String(surface_ptr, c_p, bold, &loc, -1, tickbox_ptr->caption );
		loc.x-=4;

	} else if (tickbox_ptr->skin && default_skin_ptr != NULL) {
		bold=EG_FALSE;
...
*/	if ( ( event_ptr->type == SDL_USEREVENT && event_ptr->user.code == EG_USER_SDL_TRIGGER_SELECT_DOWN ) ) {
...
/*	if (event_ptr->type == SDL_KEYUP
	 && event_ptr->key.keysym.sym == SDLK_RETURN){
*/ 	if ( ( event_ptr->type == SDL_USEREVENT && event_ptr->user.code == EG_USER_SDL_TRIGGER_SELECT_UP ) ) {
		/* If this widget currently has focus on the window.
		 */
		if (EG_Window_ThisWidgetHasFocus(widget_ptr) == EG_TRUE){
...
static void InitializeWidget(EG_Widget *widget_ptr, SDL_Color color, SDL_Rect dimension, const char *caption_ptr)
{
	...
	(void) EG_TickBox_SetCaption(widget_ptr, caption_ptr);
        ...
        (void) EG_TickBox_SetSkin(widget_ptr, NULL);
        (void) EG_TickBox_UseDefaultSkin(widget_ptr, EG_TRUE);
}
...
EG_BOOL EG_TickBox_SetSkin(EG_Widget *widget_ptr, EG_Skin_Button *skin_ptr)
{
        EG_TickBox *tickbox_ptr;
        EG_TICKBOX_GET_STRUCT_PTR(widget_ptr, tickbox_ptr, EG_FALSE);

        tickbox_ptr->skin_ptr = skin_ptr;

        return EG_TRUE;
}

EG_BOOL EG_TickBox_UseDefaultSkin(EG_Widget *widget_ptr, EG_BOOL v)
{
        EG_TickBox *tickbox_ptr;
        EG_TICKBOX_GET_STRUCT_PTR(widget_ptr, tickbox_ptr, EG_FALSE);
...
void EG_TickBox_SetDefaultSkin(EG_Skin_Button *skin_ptr)
{
        default_skin_ptr = skin_ptr;
}

void EG_TickBox_ClearDefaultSkin()
{
        return EG_TickBox_SetDefaultSkin(NULL);
}
