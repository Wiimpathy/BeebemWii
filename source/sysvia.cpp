...
void DoKbdIntCheck() {
  ...
  Oldflag=(SysVIAState.ifr & 1);
  if ((KeysDown>0) && ((SysVIAState.pcr & 0xc)==4)) {
    if ((IC32State & 8)==8) {
      SysVIAState.ifr|=1; /* CA2 */
      /*std::cerr << "DoKbdIntCheck: Caused interrupt case 1\n"; */
      UpdateIFRTopBit();
    } else {
      if (KBDCol<10) {
        int presrow;
        for(presrow=1;presrow<8;presrow++) {
          if (SysViaKbdState[KBDCol][presrow]) {
            SysVIAState.ifr|=1;
            /*std::cerr << "DoKbdIntCheck: Caused interrupt case 2\n"; */
            UpdateIFRTopBit();
          }
        } /* presrow */
      } /* KBDCol range */
...
#ifdef KBDDEBUG
  std::cerr << "DoKbdIntCheck KeysDown=" << KeysDown << "pcr & c=" << (int)(SysVIAState.pcr & 0xc);
  std::cerr << " IC32State & 8=" << (int)(IC32State & 8) << " KBDRow=" << KBDRow << "KBDCol=" << KBDCol;
  std::cerr << " oldIFRflag=" << Oldflag << " Newflag=" << (int)(SysVIAState.ifr & 1) << "\n";
#endif
} /* DoKbdIntCheck */
...
#endif
  /* std::cerr << "IC32State now=" << hex << int(IC32State) << dec << "\n"; */

  DoKbdIntCheck(); /* Should really only if write enable on KBD changes */

  /* BeebEm GP2X LEDs:
   */
  mainWin->SetCapsLED((IC32State&64)==0);
  mainWin->SetShiftLED((IC32State&128)==0);
} /* IC32Write */
...
static void SlowDataBusWrite(unsigned char Value) {
  ...
  /*std::cerr << "Slow data bus write IC32State=" << int(IC32State) << " Value=" << int(Value) << "\n";*/
  if (!(IC32State & 8)) {
    KBDRow=(Value>>4) & 7;
    KBDCol=(Value & 0xf);
    /*std::cerr << "SlowDataBusWrite to kbd  Row=" << KBDRow << " Col=" << KBDCol << "\n"; */
    DoKbdIntCheck(); /* Should really only if write enable on KBD changes */
  } /* kbd write */
...
static int SlowDataBusRead(void) {
  ...
  /* std::cerr << "SlowDataBusRead giving 0x" << hex << result << dec << "\n"; */
  return(result);
} /* SlowDataBusRead */
...
void SysVIAWrite(int Address, int Value) {
  /* std::cerr << "SysVIAWrite: Address=0x" << hex << Address << " Value=0x" << Value << dec << " at " << TotalCycles << "\n";
  DumpRegs(); */
  switch (Address) {
    case 0:
    ...
    case 14:
      /*std::cerr << "Write ier Value=" << Value << "\n"; */
      if (Value & 0x80)
...
int SysVIARead(int Address) {
  int tmp;
  /* std::cerr << "SysVIARead: Address=0x" << hex << Address << dec << " at " << TotalCycles << "\n";
  DumpRegs(); */
  switch (Address) {
    case 0: /* IRB read */
    ...
    case 13:
      UpdateIFRTopBit();
#ifdef KBDDEBUG
      std::cerr << "Read IFR got=0x" << hex << int(SysVIAState.ifr) << dec << "\n";
#endif
...
void SysVIATriggerCA1Int(int value) {
  /*value^=1; */
  /*std::cerr << "SysVIATriggerCA1Int at " << TotalCycles << "\n"; */
...
void SysVIA_poll_real(void) {
  ...
  if (SysVIAState.timer1c<0) {
    ...
    if ((SysVIAState.timer1hasshot==0) || (SysVIAState.acr & 0x40)) {
      /* std::cerr << "SysVia timer1 int at " << TotalCycles << "\n"; */
  ...
  if (SysVIAState.timer2c<0) {
    ...
    if (SysVIAState.timer2hasshot==0) {
      /* std::cerr << "SysVia timer2 int at " << TotalCycles << "\n"; */
